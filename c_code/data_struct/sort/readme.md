...
树形选择排序虽然降低了数据的比较次数,但是引用了额外的存储空间, 而且存储空间为2n. 堆排序便是在树形选择排序之上降低额外的存储空间的算法.

#### 堆排序

和树形选择排序相比堆排序只需要一个辅助空间, 堆的定义为: n和序列的元素, 对于其中任何一个元素k都有K<=2K和K<=2K+1, 对于堆的操作一般有, 获取对顶元素,向堆中插入一个元素.

不管是对堆中的数据插入还是删除, 都无法避免调整堆. 步骤如下:

1. 首先, 我们要获取最后一个非终端节点, 通常为pos=n/2-1;
2. 从pos位置不断的向后调整堆的结构.
3. 如果调整的节点有子树我们还要对子树进行调整.

代码如下: [堆排序](./heapsort.c)

堆排序在较大的数据量中有很好的性能, 因为不管是向堆中添加元素还是获取对顶元素都会将堆自平衡, 也就是说永远都维护这一个最大堆或最小堆. 
